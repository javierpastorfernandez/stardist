...
class CyclicLR(BaseModel,Callback):
    ...
    def __init__(
            self, config, name=None, basedir='.',base_lr=0.001,
            max_lr=0.006,step_size=2000.,
            mode='triangular',gamma=1.,
            scale_fn=None,scale_mode='cycle'):

        super(BaseModel, self).__init__(config=config, name=name, basedir=basedir)
...
class StarDistBase(BaseModel):
    ...
    def prepare_for_training(self, optimizer=None):
        ...
        if self.self.config.cycliclr is not None:
            self.callbacks.insert(0,self.config.cycliclr(**rlrop_params))
